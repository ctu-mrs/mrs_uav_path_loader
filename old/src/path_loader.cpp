...
private:
  ...
  void onInit() { 
    ...
    mrs_lib::ParamLoader loader(node_ptr, "PathLoaderNode");
    ...
    mrs_lib::ServiceClientHandler<mrs_msgs::srv::PathSrv> client(node_ptr, "path_out", rclcpp::SystemDefaultsQoS(), cbg);

    // | ------------------- prepare the message ------------------ |

    auto req = std::make_shared<mrs_msgs::srv::PathSrv::Request>();
